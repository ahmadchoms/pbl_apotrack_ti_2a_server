...
import { Building2, Clock, MapPin, FileText, Save, Image as ImageIcon, FileUp, Navigation, ShieldCheck, ShieldAlert, Phone, Trash2, AlertTriangle, Users } from "lucide-react";
...
export default function AdminPharmacyEdit({ pharmacy, available_staff = [] }) {
    ...
    const handleSubmit = (e) => { e.preventDefault(); put(route("admin.pharmacies.update", pharmacy.id)); };
    const handleDelete = () => { router.delete(`/admin/pharmacies/${pharmacy.id}`); };

    return (
        <DashboardAdminLayout activeMenu="pharmacies">
            <form onSubmit={handleSubmit} className="space-y-10 pb-20">
                <PageHeader subtitle="Manajemen Fasilitas" title="Edit Apotek" description={`Memperbarui data dan konfigurasi ${pharmacy.name}.`}>
                    ...
                    <Button type="submit" disabled={processing} className="h-14 px-10 rounded-2xl bg-[#0b3b60] text-white font-black text-[10px] uppercase tracking-widest hover:bg-[#082a45] transition-all shadow-xl shadow-[#0b3b60]/20 flex items-center gap-2"><Save className="w-4 h-4" />{processing ? "Menyimpan..." : "Simpan Perubahan"}</Button>
                </PageHeader>
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <motion.section variants={itemVariants} className="space-y-6">
                            ...
                            <Card className="rounded-[2.5rem] border-0 shadow-2xl shadow-slate-200/30 bg-white p-2 overflow-visible">
                                <CardContent className="p-10"><StaffPicker staffs={data.staffs} staffSearch={staffSearch} setStaffSearch={setStaffSearch} filteredAvailableStaff={filteredAvailableStaff} addStaff={addStaff} removeStaff={removeStaff} updateStaffRole={updateStaffRole} /></CardContent>
                            </Card>
                        </motion.section>
                            ...
                            <Card className="rounded-[2.5rem] border-2 border-rose-100 bg-rose-50/20 p-2">
                                <CardContent className="p-10 flex flex-col md:flex-row items-center justify-between gap-6">
                                    <div className="space-y-2">
                                        <h4 className="text-base font-black text-rose-900 leading-none">Hapus Data Apotek</h4>
                                        <p className="text-xs font-bold text-rose-600/60 leading-relaxed max-w-md">Tindakan ini bersifat permanen. Seluruh data staf, obat, riwayat, dan order akan dihapus dari sistem.</p>
                                    </div>
                                    <AlertDialog>
                                        <AlertDialogTrigger asChild><Button variant="destructive" className="h-14 px-10 rounded-2xl bg-rose-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-600/20 flex items-center gap-2 shrink-0"><Trash2 className="w-4 h-4" /> Hapus Apotek</Button></AlertDialogTrigger>
                                        <AlertDialogContent className="rounded-[2.5rem] border-0 shadow-2xl p-10 max-w-lg">
                                            <AlertDialogHeader>
                                                ...
                                                <AlertDialogDescription className="text-sm font-bold text-slate-400 text-center leading-relaxed">Apakah Anda yakin ingin menghapus <span className="text-slate-900">{pharmacy.name}</span>? Seluruh data terkait akan dihapus secara permanen.</AlertDialogDescription>
                                            </AlertDialogHeader>
                                            <AlertDialogFooter className="flex flex-row items-center justify-center gap-4 mt-10">
                                                ...
                                                <AlertDialogAction onClick={handleDelete} className="h-14 flex-1 rounded-2xl bg-rose-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-600/20">Ya, Hapus Apotek</AlertDialogAction>
                                            </AlertDialogFooter>
                                        </AlertDialogContent>
                                    </AlertDialog>
                                </CardContent>
                    ...
                    <div className="space-y-10">
                        <motion.section variants={itemVariants}>
                            <Card className="rounded-[2.5rem] border-0 shadow-2xl shadow-slate-200/40 bg-white overflow-hidden">
                                <CardContent className="p-10 space-y-8">
                                    <SectionHeader icon={<ShieldCheck className="w-5 h-5" />} bg="bg-emerald-50" color="text-emerald-600" title="Status & Verifikasi" />
                                    <FormField label="Status Verifikasi">
                                        <Select value={data.verification_status} onValueChange={(v) => setData("verification_status", v)}>
                                            <SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-transparent focus:ring-[#0b3b60]/20 font-bold"><SelectValue /></SelectTrigger>
                                            <SelectContent className="rounded-2xl border-slate-100 shadow-xl">{VERIFICATION_OPTIONS.map((o) => (<SelectItem key={o.value} value={o.value} className="text-xs font-bold">{o.label}</SelectItem>))}</SelectContent>
                                        </Select>
                                    </FormField>
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50">
                                            <div><p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Apotek Aktif</p><p className="text-[9px] text-slate-400 font-bold mt-0.5">Tampilkan di pencarian publik</p></div>
                                        ...
                                        <div className="flex items-center justify-between p-4 rounded-2xl bg-rose-50/50 border border-rose-100">
                                            <div><p className="text-[10px] font-black text-rose-700 uppercase tracking-widest">Tutup Paksa</p><p className="text-[9px] text-rose-400 font-bold mt-0.5">Override jam operasional</p></div>
                                            <Switch checked={data.is_force_closed} onCheckedChange={(v) => setData("is_force_closed", v)} />
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <motion.section variants={itemVariants}>
                            <Card className="rounded-[2.5rem] border-0 shadow-2xl shadow-slate-200/40 bg-white overflow-hidden">
                                <CardContent className="p-10 space-y-8">
                                    <SectionHeader icon={<FileUp className="w-5 h-5" />} bg="bg-slate-50" color="text-slate-400" title="Dokumen" />
                                    <UploadZone icon={<FileText className="w-6 h-6" />} label="Dokumen SIA (.PDF)" subLabel="Ganti file SIA" hint="Maks. 5MB" />
                                    <UploadZone icon={<ImageIcon className="w-6 h-6" />} label="Foto Apotek" subLabel="Ganti Foto Utama" />
                                </CardContent>
                            </Card>
                        </motion.section>
                    </div>
