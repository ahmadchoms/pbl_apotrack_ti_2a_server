    ...
    return (
        <AlertDialog open={!!user} onOpenChange={() => onClose()}>
            <AlertDialogContent className="rounded-[2.5rem] border-0 shadow-2xl p-10 max-w-lg">
                <AlertDialogHeader>
                    <div
                        className={`w-20 h-20 ${isSuspending ? "bg-rose-50 text-rose-600" : "bg-emerald-50 text-emerald-600"} rounded-[2rem] flex items-center justify-center mb-6 mx-auto`}
                    >
                        <ShieldAlert className="w-10 h-10" />
                    </div>
                    <AlertDialogTitle className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight mx-auto">
                        {isSuspending
