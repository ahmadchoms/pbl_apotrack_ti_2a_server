...
export function UserTable({ users = [], onDelete, onResetPassword }) {
    return (
        <Table>
            <TableHeader className="bg-slate-50/50">
                <TableRow className="hover:bg-transparent border-slate-100/50">
                    ...
                    <TableHead className="pr-10 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                        Aksi
                    </TableHead>
                </TableRow>
            </TableHeader>
                ...
                <AnimatePresence mode="wait">
                    {users.length === 0 ? (
                        ...
                        users.map((user) => {
                            ...
                            const statusCfg = user.is_active
                                ? USER_STATUS_CONFIG.active
                                : USER_STATUS_CONFIG.inactive;
                            return (
                                <TableRow
                                    key={user.id}
                                    className="group hover:bg-slate-50/30 transition-colors border-slate-100/50"
                                >
                                    ...
                                    <TableCell>
                                        <div
                                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[9px] font-black tracking-[0.1em] ${statusCfg.badge}`}
                                        >
                                            <span
                                                className={`w-1 h-1 rounded-full ${statusCfg.dot} animate-pulse`}
                                            />
                                            {statusCfg.label}
                                        </div>
                                    </TableCell>
                                    <TableCell className="pr-10 text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent
                                                align="end"
                                                className="rounded-2xl border-slate-200 shadow-xl w-40 p-2"
                                            >
                                                <DropdownMenuItem
                                                    onClick={() =>
                                                        router.get(
