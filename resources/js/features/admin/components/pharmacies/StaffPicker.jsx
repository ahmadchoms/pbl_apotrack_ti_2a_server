        ...
        <div className="space-y-8">
                                    ...
                                    <button key={user.id} type="button" onClick={() => addStaff(user)} className="w-full flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-colors group">
                                        <div className="flex items-center gap-3">
                                            <Avatar className="h-10 w-10 rounded-xl border border-white shadow-sm">
                                                ...
                                                <AvatarFallback className="bg-slate-100 text-slate-400 text-xs font-black">{user.username.substring(0, 2).toUpperCase()}</AvatarFallback>
                                            </Avatar>
                                            <div className="text-left">
                                                <p className="text-xs font-black text-slate-700">{user.username}</p>
                                                <p className="text-[10px] font-bold text-slate-400">{user.email}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Badge variant="secondary" className="text-[9px] font-black uppercase tracking-widest bg-slate-100 text-slate-500">{user.role}</Badge>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {staffs.length > 0 ? (
                        staffs.map((staff) => (
                            <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={staff.user_id} className="flex items-center gap-4 p-4 rounded-3xl bg-slate-50 border border-slate-100 group relative">
                                <Avatar className="h-12 w-12 rounded-2xl border-2 border-white shadow-md">
                                    <AvatarImage src={staff.user_data?.avatar_url} />
                                    <AvatarFallback className="bg-white text-slate-400 text-xs font-black">{staff.user_data?.username?.substring(0, 2).toUpperCase()}</AvatarFallback>
                                </Avatar>
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-black text-slate-900 truncate">{staff.user_data?.username}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <button type="button" onClick={() => updateStaffRole(staff.user_id, "APOTEKER")} className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-colors ${staff.role === "APOTEKER" ? "bg-[#0b3b60] text-white" : "bg-white text-slate-400 border border-slate-100 hover:bg-slate-100"}`}>Apoteker</button>
                                        <button type="button" onClick={() => updateStaffRole(staff.user_id, "STAFF")} className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-colors ${staff.role === "STAFF" ? "bg-[#0b3b60] text-white" : "bg-white text-slate-400 border border-slate-100 hover:bg-slate-100"}`}>Staf</button>
                                    </div>
                                </div>
                                <button type="button" onClick={() => removeStaff(staff.user_id)} className="w-8 h-8 rounded-xl bg-white text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-all shadow-sm flex items-center justify-center opacity-0 group-hover:opacity-100">
                                    <X className="w-4 h-4" />
                                </button>
                            </motion.div>
                        ))
                        ...
                        <div className="col-span-full py-10 border-2 border-dashed border-slate-100 rounded-[2rem] flex flex-col items-center justify-center text-slate-300">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest">Belum ada staf ditambahkan</p>
                        </div>
                    )}
                </div>
            </div>
