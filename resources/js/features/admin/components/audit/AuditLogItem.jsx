            ...
            <CardContent className="p-6">
                <div className="flex flex-col md:flex-row items-center gap-6">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                        <div
                            className={`w-10 h-10 rounded-xl ${colors} flex items-center justify-center shrink-0`}
                        >
                            <Icon className="w-5 h-5" />
                        </div>
                        <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <h4 className="text-sm font-black text-slate-900 truncate tracking-tight uppercase">
                                    {log.action.replace(/_/g, " ")}
                                </h4>
                                <Badge
                                    className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-lg border-0 ${log.status === "SUCCESS" ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}
                                >
                                    {log.status === "SUCCESS"
                                        ? "Berhasil"
                                        : "Gagal"}
