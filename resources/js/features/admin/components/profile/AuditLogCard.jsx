...
const ACTION_MAP = {
    LOGIN: { icon: LogIn, color: "text-primary", bg: "bg-blue-50" },
    PROFILE_UPDATE: {
        icon: Info,
        ...
        <Card className="border-0 shadow-2xl shadow-slate-200/50 rounded-[2.5rem] overflow-hidden bg-white group flex flex-col h-full">
            ...
            <CardContent className="p-10 pt-4 flex-1">
                    ...
                    {logs.length > 0 ? (
                        logs.map((log) => {
                            ...
                            return (
                                ...
                                >
                                    <div
                                        className={`absolute left-0 top-1 w-3 h-3 rounded-full border-2 border-white shadow-sm transition-transform group-hover/item:scale-125 bg-slate-200`}
                                    ></div>
                ...
                <Link
                    ...
                    className="w-full mt-10 h-14 rounded-2xl bg-slate-50 hover:bg-primary text-slate-500 hover:text-white text-[10px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 group/btn"
                >
