...
export function OrderFilter({ search, onSearchChange, selectedStatuses, onStatusChange, onClearFilters }) {
    const statuses = [
    ...
    return (
        <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
                <InputGroup className="rounded-2xl bg-white shadow-sm border-slate-200 focus-within:border-[#00346C]/40 transition-all">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="outline" className="h-12 px-5 rounded-2xl border-slate-200 bg-white hover:bg-slate-50 font-bold text-slate-600 gap-2 shadow-sm">
                    ...
                    <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 shadow-xl border-slate-100">
                        ...
                        {statuses.map((status) => (
                            <DropdownMenuCheckboxItem
                                ...
                                className="rounded-xl text-xs font-bold text-slate-600 py-2.5 focus:bg-slate-50 focus:text-[#00346C]"
                            >
                                {status.label}
                            </DropdownMenuCheckboxItem>
                        ))}
