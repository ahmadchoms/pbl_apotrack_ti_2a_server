        ...
        <Card className="border-0 shadow-lg shadow-slate-200/50 rounded-3xl overflow-hidden bg-white">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-slate-50 flex flex-row items-center justify-between gap-3">
                ...
                <Button
                    ...
                    disabled={processing}
                    className="bg-primary hover:bg-[#082a45] text-white rounded-xl h-10 px-6 flex items-center gap-2"
                >
            ...
            <CardContent className="px-8 py-6">
                <div className="space-y-4">
                    {DAYS.map((day, index) => {
                        const hourData = data.hours.find(
                        ...
                        const dataIndex = data.hours.findIndex(
                            (h) => h.day_of_week === index,
                        );
                        const effectiveIndex =
                            dataIndex === -1 ? data.hours.length : dataIndex;

                        return (
                            <div
                                key={day}
                                className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50 rounded-2xl gap-4 border border-slate-100"
                            >
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                ...
                                                value={hourData.open_time?.substring(
                                                    0,
                                                    5,
                                                )}
                                                onChange={(e) =>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                ...
                                                value={hourData.close_time?.substring(
                                                    0,
                                                    5,
                                                )}
                                                onChange={(e) =>
