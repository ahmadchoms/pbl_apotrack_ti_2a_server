    ...
    const toggleStatus = (val) => {
        router.patch(
            ...
            { is_active: val },
            {
                onSuccess: () =>
                    toast.success(
                        `Status apotek diubah ke ${val ? "Buka" : "Tutup"}`,
                    ),
            },
        );
    };
        ...
        <Card className="border-0 shadow-lg shadow-slate-200/50 rounded-3xl overflow-hidden hover:scale-[1.01] transition-transform duration-300 bg-white">
            <CardContent className="p-8 flex flex-col items-center relative">
                ...
                >
                    <DialogTrigger asChild>
                        <Button
                            variant="ghost"
                            size="sm"
                            className="absolute top-4 right-4 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-full"
                        >
                        ...
                        <div className="space-y-5 py-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full flex min-h-[80px] rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
                                />
                            </div>
                        </div>
                        <DialogFooter>
                            <Button
                                ...
                                disabled={processing}
                                className="bg-primary hover:bg-[#082a45] text-white rounded-xl px-6 w-full sm:w-auto"
                            >
                                {processing
                                    ? "Menyimpan..."
                                    : "Simpan Perubahan"}
                ...
                <div className="relative mb-4">
                    ...
                    <Avatar className="h-28 w-28 border-4 border-white shadow-md relative z-10">
                        <AvatarImage
                            src={
                                user?.avatar_url ||
                                `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.username}&backgroundColor=0b3b60`
                            }
                        />
                    ...
                    <div className="absolute bottom-0 right-0 bg-primary p-1.5 rounded-full border-2 border-white z-20 text-white shadow-sm">
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                    {user?.pharmacy_staff?.role?.replace("_", " ") ||
                        "Petugas Apotek"}
                </p>
